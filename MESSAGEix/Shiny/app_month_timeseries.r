  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "dataset",
                  ...
                  choices = c('Energy production by source','Final energy','Water widthrawals by source','Final water use',
                              ...
                              'Activity','Input',"Investment cost",'Output',"Demand",'Emission','Storage','Storage_chg'
                              ) ),
    ...
    mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel("Table",value=3, downloadButton("downloadData", "Download"),
                           tableOutput("table")
                           )
      )
...
server <- function(input, output,session) {
  ...
  datasetInput <- reactive({
    switch(input$dataset,
           ...
           'Storage' = raw_extraction$STORAGE,
           'Storage_chg' = raw_extraction$STORAGE_CHG
           )
  })
observe({
  
  group_chs <- (datasetInput() %>% names() %>% as.data.frame() %>% filter(!. %in% c('time','value')))$. 
  ...
  output$plot <- renderPlot({
    ...
    plot_data <- datasetInput() %>% 
      ...
      group_by_('time',input$grouping,input$facet) %>% 
    ...
    plot1 <<- ggplot(plot_data)+
       geom_line(aes_string(x = 'time', y = 'value', color = input$grouping),size = 2)+ 
      facet_wrap(as.formula(paste("~", input$facet)))+
      theme_bw() + xlab('Months') + ylim(0,max(plot_data$value))+
      theme(legend.position = 'bottom',
            axis.text = element_text(size = 12))
    } else {
      plot1 <<- ggplot(plot_data)+
        geom_bar(aes_string(x = 'time', y = 'value', fill = input$grouping), stat = "identity", position = "stack")+ 
        facet_wrap(as.formula(paste("~", input$facet)))+
        theme_bw() + xlab('Months') + 
        theme(legend.position = 'bottom',
              axis.text = element_text(size = 12))
    }
    plot1

...
Sets
    ...
    seq_time(time,time2)             mapping of one time period ('time') to the next ('time2')
    first_time(time)                 flag for first time in model horizon
    last_time(time)                  flag for last time in model horizon
    seq_year_time(year_all,year_all2,time,time2)
;
...
set subtime(time) /1*12/;
alias (subtime,subtime2);
*loop(time,
*subtime(time) = yes$map_temporal_hierarchy('sub_1',time,'year')
*);
seq_time(subtime,subtime2)$( ORD(subtime) + 1 = ORD(subtime2) ) = yes ;
seq_year_time(year_all,year_all2,subtime,subtime2)$(ORD(year_all) = ORD(year_all2) AND   seq_time(subtime,subtime2)) = YES;
* define first and last time periods
first_time(subtime)$( ORD(subtime) = 1) = yes ;
last_time(subtime)$( ORD(subtime) = CARD(subtime) ) = yes ;
seq_year_time(year_all,year_all2,last_time(subtime),first_time(subtime2))$(seq_period(year_all,year_all2)) = YES;
